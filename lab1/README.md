# Лабораторная работа №1
## Вариант 121058
## Задание
1. Для указанной функции провести модульное тестирование разложения функции в степенной ряд. Выбрать достаточное тестовое покрытие.
> функция: arsin(x)
2. Провести модульное тестирование указанного алгоритма. Для этого выбрать характерные точки внутри алгоритма, и для предложенных самостоятельно наборов исходных данных записать последовательность попадания в характерные точки. Сравнить последовательность попадания с эталонной.
> Алгоритм: Программный модуль для поиска кратчайшего пути в ориентированном графе по алгоритму Дейкстры (http://www.cs.usfca.edu/~galles/visualization/Dijkstra.html)
3. Сформировать доменную модель для заданного текста.  Разработать тестовое покрытие для данной доменной модели
> Описание предметной области: Марвин наблюдал за ней с холодным презрением. Его логические схемы в отвращении застрекотали и начали манипулировать идеей применить к двери физическое насилие. Затем они щелкнули и сказали: "А стоит ли обращать внимание? Ради чего?". Тогда схемы решили позабавиться, произведя сравнительный анализ молекулярных компонентов двери и клеток человеческого мозга. После этого они на бис измерили уровень водорода в близлежащем кубическом парсеке космоса, а потом им все надоело, и они отключились. С судорогой отчаяния робот повернулся.
## Выполнение
### Модульное тестирование разложения функции в степенной ряд
Функция по варианту: arcsin(x)
график: 
![arcsin(x) plot](/static/plot.png)
#### Определение тестового покрытия
Выделим промежутки эквивалентности и граничные точки:
1) при |x| > 1 функция не определена
2) при x = 0, asin = 0
3) функция имеет промежутки эквивалентности при -1 <= x < 0 и 0 < x <= 1
#### Определение целевой функции
Из курса математического анализа:
![arcsin(x) formula](/static/asinFormula.png)
Поскольку считать факториалы долго и сложно, упростим формулу нахождения следующего члена ряда Тейлора, так, если найдем отношение: n-го члена и n-1-го, то получим формулу:
![terms ratio ](/static/Simple%20formula.png)
Таким образом, мы находим текущее слагаемое, складываем с итоговой суммой.
Продолжаем пока не достигнем точности или не достигнем максимума итераций.
### Модульное тестирование указанного алгоритма
Алгорим по варианту: Алгоритм Дейкстры
#### Определение тестового покрытия
1) Граф из одной вершины
2) Путя от начальной вершины до конечной не существует
3) Находим кратчайший путь сразу
4) Находим сначала длинный путь, а потом короче
Краткое описание алгоритма:
Алгоритм Дейкстры пошаговый. Сначала выбирается точка, от которой будут отсчитываться пути. Затем алгоритм поочередно ищет самые короткие маршруты из исходной точки в другие. Вершины, где он уже побывал, отмечает посещенными. Алгоритм использует посещенные вершины, когда рассчитывает пути для непосещенных.
### Тестирование доменной области
Описание предметной области: Марвин наблюдал за ней с холодным презрением. Его логические схемы в отвращении застрекотали и начали манипулировать идеей применить к двери физическое насилие. Затем они щелкнули и сказали: "А стоит ли обращать внимание? Ради чего?". Тогда схемы решили позабавиться, произведя сравнительный анализ молекулярных компонентов двери и клеток человеческого мозга. После этого они на бис измерили уровень водорода в близлежащем кубическом парсеке космоса, а потом им все надоело, и они отключились. С судорогой отчаяния робот повернулся.